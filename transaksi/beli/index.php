...
<div class="container">
	...
	<div class="main">
		...
		include '../koneksi.php';
		$nomor = 1;
		$sql = "SELECT pelanggan.nama_pelanggan AS kode_konsumen, produk.nama_produk AS kode_produk, beli.kode_transaksi, beli.jumlah_produk, beli.total_harga   
FROM ((beli
INNER JOIN pelanggan ON beli.kode_konsumen = pelanggan.kode_pelanggan)
INNER JOIN produk ON beli.kode_produk = produk.kode_produk)";
		$result = mysqli_query($connect,$sql);
		function rupiah($R){
		$after = "Rp " . number_format($R,0,'','.');
		return $after;
		}
		if (mysqli_num_rows($result)>0){
			...
			echo "<tr>
							...
							<td><a href='edit.php?id=".$row['kode_transaksi']."'>Edit</a> |
									<a href='delete.php?id=".$row['kode_transaksi']."'onclick='javascript:return confirm(\"Apakah anda yakin ingin menghapus data ini?\")'>Hapus</a>			
							</td>
						</tr>";
			}
		}
