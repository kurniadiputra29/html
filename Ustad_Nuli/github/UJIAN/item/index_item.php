...
table {
	...
	<div id="container">
		...
		<div class="table">
				<ul>
				<li><a href="create_item.php">Add</a></li>
				</ul>
				<table>
					<tr>
						<th>No</th>
			...
			include '../koneksi.php';
			$nomor  = 1;
			$sql    = "SELECT item.id,category.name AS category,item.name,item.price,item.status
								FROM item 
								INNER JOIN category
								ON
			...
			function jika($m){// untuk merubah boolean 
			if ($m==1){
				return "Aktif";
			} else {
				return "Tidak Aktif";
			...
			if(mysqli_num_rows($result)>0){// jika nggak ada datanya maka while tidak di jalankan
				while ($row = mysqli_fetch_assoc($result)) {// untuk memunculkan dalam bentuk rapi, mengambil dan dijadikan erray associative
					...
					<tr>
						...
						<td>
							<a href='edit_item.php?id=".$row['id']."'>Update</a> |
							<a href='delete_item.php?id=".$row['id']."' onclick='javascript:return confirm(\"Apakah anda yakin ingin menghapus data ini?\")'>Delete</a>
						</td>
					</tr>
					";
				}
