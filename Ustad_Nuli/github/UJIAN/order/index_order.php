...
table {
	...
	<div id="container">
		...
		<div id="header2">INDEX ORDER</div>
		<div class="menu">
		<ul>
		...
		<div class="table">
				<ul>
				<li><a href="create_order.php">Add</a></li>
				</ul>
				<table>
					<tr>
						<th>No</th>
					...
					include '../koneksi.php';
					$nomor  = 1;
					$sql    = "SELECT orders.id, orders.table_number, item.name AS item_id, orders.qty, orders.total, orders.diskon, orders.after_diskon   
										FROM orders 
										JOIN item
										ON
					...
					function rupiah($angka){ // untuk membuat format rupiah
					...
					if(mysqli_num_rows($result)>0){// jika nggak ada datanya maka while tidak di jalankan
						while ($row = mysqli_fetch_assoc($result)) {// untuk memunculkan dalam bentuk rapi, mengambil dan dijadikan erray associative
							...
							<tr>
								...
								<td>
									<a href='edit_order.php?id=".$row['id']."'>Update</a> |
									<a href='delete_order.php?id=".$row['id']."' onclick='javascript:return confirm(\"Apakah anda yakin ingin menghapus data ini?\")'>Delete</a>
								</td>
							</tr>
							";
						}
